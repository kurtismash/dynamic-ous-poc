...
resource "aws_organizations_policy" "scp_restrict_regions" {
  ...
  for_each = { for region_list in distinct([for ou in local.ous_with_project_data : sort(lookup(ou.controls, "allow_regions", []))]) : join(",", region_list) => region_list if length(region_list) > 0 }
...
resource "aws_organizations_policy_attachment" "scp_restrict_regions_attachment" {
  ...
  for_each = { for k, ou in local.ous_with_project_data : k => join(",", sort(ou.controls.allow_regions)) if try(length(ou.controls.allow_regions), 0) > 0 }
